        ...
        if ($this->process == 0) {
            ...
            if ($syncTerm instanceof SyncTerminal) {
                ...
                foreach ($reader->getSheetIterator() as $sheet) {
                    ...
                    foreach ($sheet->getRowIterator() as $row) {
                        ...
                        if ($count > 2) {
                            ...
                            if (($count % Yii::$app->params['appTmsSyncProcess']) == 0) {
                                Yii::$app->queue->priority(5)->push(new SyncTerminalParameter([
                                    'queueLog' => strVal(round(microtime(true)*1000)),
                                    'process' => 2,
                                    'user' => $this->user,
                    ...
                    Yii::$app->queue->priority(1)->push(new SyncTerminalParameter([
                        'queueLog' => strVal(round(microtime(true)*1000)),
                        'process' => 2,
                        'user' => $this->user,
                        'syncId' => $syncTerm->sync_term_id,
...
echo var_dump("masuk");
                            ...
                            echo var_dump("masuk sini".$data[0]);
		...
		//Yii::$app->cache->delete('report_submitted_by_user_' . $userId);
                }
            }
        }
    }
