...
<div class="veristore-terminal">
    ...
    <div class="form-group">
        <div class="row">
            ...
            <div class="col-lg-1">
            <?php
                   $isDisabled = Yii::$app->cache->get('report_submitted_by_user_' . Yii::$app->user->id) ?: false;

                   // Tambahkan pengecekan status terminal seperti sebelumnya
                   if (Yii::$app->user->identity->user_privileges != 'TMS SUPERVISOR') {
                       $hasPendingSync = SyncTerminal::find()->where(['sync_term_status' => ['0', '1', '2']])->count() > 0;
                   
                       $btnClass = 'glyphicon glyphicon-list-alt btn btn-success';
                       if ($hasPendingSync || $isDisabled) {
                           $btnClass .= ' disabled';
                       }
                   
                       echo Html::a(' Update', ['report'], [
                           'class' => $btnClass,
                           'data-pjax' => 0,
                           'onclick' => '$("#spinImportReport").removeClass("kv-hide");'
                       ]);
                   }
                    ...
                    <tr>
                        <td style="width:35%;">
                            <?= $form->field($model, 'searchType')->widget(Select2::classname(), [
                                'data' => [4 => 'CSI', 1 => "Merchant Name", 2 => "Group Name", 3 => "TID", 5 => "MID", 0 => "SN"],
                                'hideSearch' => true,
                                'options' => [
                                    'id' => 'id_select_search',
                        ...
                        <td>
                            <?= $form->field($model, 'serialNo')->textInput(['id' => 'serialNo', 'placeholder' => 'CSI'])->label(false) ?>
                        </td>
                    </tr>
                </table>
            </div>
...
//    ");
    ...
    <?php Pjax::end(); ?>
    <?php
    if (Yii::$app->session->hasFlash('reportSubmitted')) {
        $this->registerJs('location.reload();', \yii\web\View::POS_READY);
    }
?>
