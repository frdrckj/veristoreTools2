...
class VeristoreController extends Controller {
    ...
    public function actionTerminal() {
        //$selectAllList = TmsHelper::getTerminalListToFile();
    ...
    if (!is_null($response)) {
        ...
        $searchSelectAllList = [];
        $searchSelectAllList[$page - 1] = rtrim($tmpList, '|');
    } else {
    ...
    if (isset(Yii::$app->request->post()['selectAllList'])) {
        ...
        $totalAllList = Yii::$app->request->post('totalAllList', 0);
    } else if (isset(Yii::$app->request->get()['selectAllList'])) {
        $selectAllList = Yii::$app->request->get()['selectAllList'];
        $totalAllList = Yii::$app->request->get('totalAllList', 0);
    } else {
        $totalAllList = 0;
        $selectAllList = '[]';

        // Ambil dari file jika ada
        ...
        if ((Yii::$app->request->isPost) && ($model->load(Yii::$app->request->post()))) {
            ...
            if (!is_null($response)) {
                if (intval($response['resultCode']) == 0) {
                    ...
                    if (!is_null($response)) {
                        ActivityLogHelper::add(ActivityLogHelper::VERISTORE_ADD_TERMINAL, 'Add csi ' . $model->deviceId);
                        Yii::$app->session->setFlash('info', 'Add CSI ' . $model->deviceId . ' berhasil!');
                        return $this->redirect('terminal');
                    } else {
    ...
    public function actionEdit($serialNo = null) { //NOSONAR
        ...
        $model = new Terminal();
        $appData = [];
            $response = TmsHelper::getAppListSearch(Yii::$app->user->identity->tms_session, $serialNo);
                foreach ($response['appList'] as $tmp) {
                    if ($tmp['packageName'] == Yii::$app->params['appTmsPackageName']) {
                        $appData[$tmp['version']] = $tmp['version'];
                    }
                }
            krsort($appData);
        if (!is_null($serialNo)) {
    ...
    public function actionReport($name = null) {
        ...
        if (Yii::$app->request->isPost) {
            if ($model->load(Yii::$app->request->post())) {
                Yii::$app->queue->priority(120)->delay(10)->push(new ReportTerminal([
